...
class AbstractBaseModel(Base):
    ...
    def created_at_kst(self) -> datetime:
        """
        KST(한국 표준시)로 변환된 생성 타임스탬프를 반환합니다.

        Returns:
            datetime: KST로 변환된 생성 타임스탬프
        """
        return self.created_at.astimezone(ZoneInfo("Asia/Seoul"))

    def updated_at_kst(self) -> datetime:
        """
        KST(한국 표준시)로 변환된 수정 타임스탬프를 반환합니다.

        Returns:
            datetime: KST로 변환된 수정 타임스탬프
        """
        return self.updated_at.astimezone(ZoneInfo("Asia/Seoul"))
