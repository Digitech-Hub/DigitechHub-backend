...
def get_sync_database() -> Generator[Session, None, None]:
    """
    동기 데이터베이스 세션을 생성하고 반환하는 제너레이터 함수입니다.
    사용 후 세션을 닫아 리소스를 해제합니다.
    
    Yields:
        Session: 동기 SQLAlchemy 데이터베이스 세션
    """
...
def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> dict:
    ...
    try:
        ...
        logger.info(f"User authenticated: {user_info['username']} with role: {user_info['role']}")
        return user_info
...
def require_role(required_role: str):
    ...
    def role_checker(current_user: dict = Depends(get_current_user)) -> dict:
        ...
        if user_role != required_role:
            logger.warning(f"Access denied for user {current_user['username']} with role {user_role}. Required: {required_role}")
            raise HTTPException(
