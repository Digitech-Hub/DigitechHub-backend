...
class EquipmentStatusInitializer:
    ...
    def initialize_default_statuses(cls, session: Session) -> list[EquipmentStatus]:
        ...
        for status_data in cls.DEFAULT_STATUSES:
            # 이미 존재하는지 확인
            existing_status = session.query(EquipmentStatus).filter(
                EquipmentStatus.name == status_data["name"]
            ).first()
        ...
        try:
            session.commit()
            print(f"Successfully initialized {len(created_statuses)} equipment statuses")
    ...
    def get_status_by_name(cls, session: Session, name: str) -> EquipmentStatus | None:
        ...
        """
        return session.query(EquipmentStatus).filter(
            EquipmentStatus.name == name
        ).first()
    ...
    def get_available_statuses(cls, session: Session) -> list[EquipmentStatus]:
        ...
        """
        return session.query(EquipmentStatus).filter(
            EquipmentStatus.is_available == True,
            EquipmentStatus.is_active == True
        ).order_by(EquipmentStatus.priority).all()
